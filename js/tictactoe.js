...
// this function will roll for random munber twice, one for
...
function rollForTurn() {
    ...
    function diceRoll() { // play dice sounds during the game roll for turn
...
// as squares qre selected they check in with this function to see if that particular 
...
function checkForWinCon() {
    var squareArray = [];
    var target = document.getElementById('boardState');
    var info = target.innerHTML; // raw array with squares and aatars
...
function check4Tie() {
    ...
    boardState = goardState.substring(1); // remove leading comma
...
function winner(winDetected,winCon) {
    if (winDetected == "win") {
        ...
        glowBoard(winCon); // call dunction to make the gameboard pulse with colors
        }
}

// this function will make the winning squares light up in a celebration 
function glowBoard(pos) {
    ...
    for (var i=0;i<squares.length;i++) {
        if (i == index0) {
            ...
            setTimeout(function() {bg2.style.backgroundColor = 'rgb(234, 179, 66)';}, 500);
...
// these functions will produce game sounds depending on the occasiion
...
function checkWinCon1(info,squareArray) {
    ...
    //index 0k index 1, and index 2 and once they
    ...
    // this will trigger (ONLY) if there was a math for index0, index1, and index2
    if (match0Avatar != undefined && match1Avatar != undefined && match2Avatar != undefined) {
        if (match0Avatar == match1Avatar && match0Avatar == match2Avatar) {
            winDetected = "win"; // this flag will pass when a win has beenb detected
